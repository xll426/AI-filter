#!/usr/bin/env python3
"""导出 teacher_qat_w10_b12 为 raw-Y residual ONNX。

不使用命令行参数。需要改路径时只改下面配置区。

ONNX 只输出 delta_y，不在图里把 delta 加回原始 Y：
...
CONFIG_PATH = "/mnt/d/fudan/prefilter_clean/deliver_w10_b13/configs/teacher_qat_w10_b12.yaml"
CHECKPOINT_PATH = "/mnt/d/fudan/prefilter_clean/deliver_w10_b13/runs/teacher_qat_w10_b12/checkpoints/best.pt"
...
class ExportableQuantPrefilterYDelta(nn.Module):
    """ONNX 图：raw Y 输入，raw delta_y 输出。"""
...
def main() -> int:
    ...
    meta = {
        ...
        "model_name": "teacher_qat_w10_b12",
