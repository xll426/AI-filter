...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Decode H.265, generate ref targets, and write train/val/test/img|gt folders.")
    parser.add_argument("--input_dir", required=True, help="Directory containing raw .h265/.265/.hevc files.")
    parser.add_argument("--output_dir", required=True, help="Output dataset root.")
    parser.add_argument("--roi_dir", default=None, help="Optional directory with per-frame ROI npy files.")
    parser.add_argument("--detect_roi", action="store_true", help="Detect face/plate ROI directly from decoded frames.")
    parser.add_argument("--roi_face_model", default=DEFAULT_FACE_MODEL, help="YOLO face detector weight path.")
    parser.add_argument("--roi_plate_model", default=DEFAULT_PLATE_MODEL, help="YOLO plate detector weight path.")
    parser.add_argument("--roi_conf", type=float, default=0.3, help="YOLO confidence threshold.")
    parser.add_argument("--roi_iou", type=float, default=0.25, help="YOLO NMS IoU threshold.")
    parser.add_argument("--roi_save_vis", action="store_true", help="Save ROI visualization JPGs under output_dir/roi_vis.")
    parser.add_argument("--frames_per_video", type=int, default=12, help="Uniformly sampled frames per video.")
    ...
    parser.add_argument("--tile_size", type=int, default=512, help="0 means keep full frame.")
    parser.add_argument("--tile_stride", type=int, default=None, help="Defaults to tile_size.")
    parser.add_argument("--pix_fmt", default="yuv420p", help="ffmpeg decode pixel format.")
    parser.add_argument("--yuv_format", default="yuv420p", help="Stored planar YUV format.")
    parser.add_argument("--bitdepth", type=int, default=8)
    parser.add_argument("--max_videos", type=int, default=None)
    parser.add_argument("--workers", type=int, default=1, help="Parallel workers over source videos.")
