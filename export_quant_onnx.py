...
class ExportableQuantPrefilterY(nn.Module):
    """ONNX-friendly Y-plane deploy graph using frozen exported integer parameters."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y_u_out = torch.clamp(torch.round(y_u + delta), 0.0, 255.0)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export int-QAT W12 Prefilter Y-plane model to ONNX.")
    ...
    parser.add_argument(
        "--output",
        default="runs/xlx_clean_roi_512_edge_aux_int_qat_w12/onnx/quant_w12_best_y.onnx",
    )
    ...
    parser.add_argument("--raw_io", action="store_true", help="Export raw 0..255 float input/output instead of normalized 0..1 I/O.")
    parser.add_argument("--check", action="store_true", help="Run torch-vs-export-module check before saving.")
...
def main() -> int:
    ...
    if args.check:
        with torch.no_grad():
            ref = qat_model(dummy / 255.0) * 255.0 if args.raw_io else qat_model(dummy)
            got = export_model(dummy)
